 ...
 * Decomposed: schema in creatorFormSchema.ts, draft loading in useCreatorDraftLoader.ts.
 */
...
export function ChallengeCreatorForm({ engagementModel, governanceMode, industrySegmentId, onDraftModeSync, onFillTestData, onDraftIdChange }: ChallengeCreatorFormProps) {
  ...
  useCreatorDraftLoader(draftChallengeId, form, governanceMode, engagementModel, onDraftModeSync);

  // Sync solution_maturity_id when maturity_level changes
    ...
    try {
      ...
      if (result.challengeId && attachedFiles.length > 0 && currentOrg?.organizationId) {
        for (const file of attachedFiles) {
          ...
          const { error: uploadError } = await supabase.storage.from('challenge-attachments').upload(storagePath, file, { upsert: false, cacheControl: '3600' });
          if (!uploadError) {
            await supabase.from('challenge_attachments').insert({
              challenge_id: result.challengeId, section_key: 'creator_reference', source_type: 'file',
              storage_path: storagePath, file_name: file.name, file_size: file.size, mime_type: file.type, uploaded_by: user?.id ?? null,
            });
          }
        }
      }
      if (isQuick) {
        toast.success('Challenge published! Solvers can now discover and apply.');
      } else {
  ...
  }, [buildPayload, submitMutation, referenceUrls, draftChallengeId, attachedFiles, currentOrg, user, navigate, isQuick]);
  ...
  }, [form, currentOrg, user, engagementModel, governanceMode, industrySegmentId, draftChallengeId, updateDraftMutation, draftMutation, onDraftIdChange]);
  ...
  const handleFillTestData = useCallback(() => {
    ...
    const seedWithDomainTags = {
      ...seed, domain_tags: domainIds, maturity_level: maturityMatch?.code ?? seed.maturity_level,
      solution_maturity_id: maturityMatch?.id ?? '', industry_segment_id: industrySegments[0]?.id ?? '',
    };
    const filteredSeed = fieldRules ? filterSeedByGovernance(seedWithDomainTags, fieldRules) : seedWithDomainTags;
    form.reset(filteredSeed as CreatorFormValues);
    onFillTestData?.();
    setTimeout(async () => { await handleSaveDraft(); toast.success('Test data filled & saved as draft'); }, 150);
  ...
  return (
    <FormProvider {...form}>
      <form onSubmit={handleSubmit} className="space-y-6">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="essential" className="mt-6">
            <EssentialDetailsTab engagementModel={engagementModel} industrySegments={industrySegments} governanceMode={governanceMode} fieldRules={fieldRules} />
          </TabsContent>
          <TabsContent value="context" className="mt-6">
            <AdditionalContextTab governanceMode={governanceMode} fieldRules={fieldRules} attachedFiles={attachedFiles} onFilesChange={setAttachedFiles} referenceUrls={referenceUrls} onUrlsChange={setReferenceUrls} />
          </TabsContent>
        </Tabs>

        <div className="flex items-center justify-between gap-3 pt-4 border-t border-border">
          <Button type="button" variant="ghost" size="sm" className="text-muted-foreground" onClick={handleFillTestData}>
            <FlaskConical className="h-4 w-4 mr-1.5" />Fill Test Data
          </Button>
          <div className="flex items-center gap-3 ml-auto">
            ...
            <Button type="button" variant={isControlled ? 'default' : 'outline'} onClick={async () => { await handleSaveDraft(); setShowAIReview(true); }} disabled={isBusy || !draftChallengeId} className="gap-1.5">
              <Sparkles className="h-4 w-4" />AI Review
              {isControlled && <Badge variant="secondary" className="ml-1 text-[10px]">Required</Badge>}
              {!isControlled && !isQuick && <Badge variant="outline" className="ml-1 text-[10px]">Recommended</Badge>}
            </Button>
            <Button type="submit" disabled={isBusy || (isControlled && !aiReviewCompleted)}>
              {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin mr-1.5" /> : <Send className="h-4 w-4 mr-1.5" />}
              {isQuick ? 'Submit & Publish' : 'Submit to Curator'}
            </Button>
          </div>
        </div>
      </form>
      ...
      {showTierModal && tierLimit && (
        <TierLimitModal isOpen={showTierModal} onClose={() => setShowTierModal(false)} tierName={tierLimit.tier_name} maxAllowed={tierLimit.max_allowed} currentActive={tierLimit.current_active} />
      )}
      {showLegalGate && (
        <LegalGateModal triggerEvent="CHALLENGE_SUBMIT" onAllAccepted={handleLegalAccepted} onDeclined={() => { setShowLegalGate(false); setPendingSubmitData(null); toast.error('Submission cancelled — legal agreement declined'); }} />
      )}
      {showAIReview && draftChallengeId && (
        <CreatorAIReviewDrawer open={showAIReview} onClose={() => setShowAIReview(false)} challengeId={draftChallengeId} governanceMode={governanceMode} engagementModel={engagementModel} industrySegmentId={industrySegmentId} onReviewComplete={() => setAiReviewCompleted(true)} />
      )}
    </FormProvider>
  );
}
