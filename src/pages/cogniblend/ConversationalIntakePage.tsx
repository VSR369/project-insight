...
export function ConversationalIntakeContent({
  ...
  const handleGenerateWithAI = async (data: IntakeFormValues) => {
    ...
    try {
      const { challengeId } = await createChallenge.mutateAsync({
        ...
        expectedOutcomes: data.expected_outcomes ? [data.expected_outcomes] : [],
        currency: data.currency_code,
        ...
        rootCauses: data.root_causes?.trim() ? [data.root_causes.trim()] : undefined,
        affectedStakeholders: data.affected_stakeholders?.trim() ? [{ stakeholder_name: data.affected_stakeholders.trim(), role: '', impact_description: '', adoption_challenge: '' }] : undefined,
        scopeDefinition: data.scope_definition?.trim() || undefined,
        preferredApproach: data.preferred_approach?.trim() ? [data.preferred_approach.trim()] : undefined,
        approachesNotOfInterest: data.approaches_not_of_interest?.trim() ? [data.approaches_not_of_interest.trim()] : undefined,
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      {/* Step 1: Template Selector (all modes: full grid) */}
      <TemplateSelector
