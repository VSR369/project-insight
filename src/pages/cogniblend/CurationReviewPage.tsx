 ...
 *    converged from Creator, Legal Coordinator, and Finance Coordinator.
...
const SECTIONS: SectionDef[] = [
      ...
      return (
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            {ch.complexity_level && (
              <Badge variant="secondary" className="text-xs capitalize">{ch.complexity_level}</Badge>
            )}
          </div>
  ...
  },
  {
    ...
    render: (ch) => (
      <p className="text-sm font-medium text-foreground">{ch.ip_model || "—"}</p>
    ),
  },
  {
    key: "domain_tags",
  ...
  /* ── Legal Documents — full detail from LC ── */
  {
    key: "legal_docs",
    ...
    render: (_ch, _legalDocs, legalDetails) => {
      if (!legalDetails || legalDetails.length === 0)
        ...
        <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
              <div>
                ...
                <p className="font-medium text-foreground">
                  {escrow.currency ?? "$"} {escrow.deposit_amount.toLocaleString()}
                </p>
              </div>
              {escrow.bank_name && (
                <div>
          ...
          {!isFunded && (
            <p className="text-xs text-amber-700">
              Finance Coordinator has not yet confirmed the deposit.
            </p>
          )}
        </div>
      );
  ...
  },
  {
    ...
    render: (ch) =>
      ch.maturity_level ? (
        <Badge variant="secondary" className="capitalize">{ch.maturity_level}</Badge>
      ) : (
  ...
  },
  {
    ...
    render: (ch) => {
      if (!ch.visibility && !ch.eligibility)
        return <p className="text-sm text-muted-foreground italic">Not yet configured</p>;
      return (
        <div className="space-y-2">
          {ch.visibility && (
            <div>
              <p className="text-xs text-muted-foreground">Visibility</p>
              <p className="text-sm text-foreground capitalize">{ch.visibility}</p>
            </div>
          )}
          {ch.eligibility && (
            <div>
...
export default function CurationReviewPage() {
  ...
  // ══════════════════════════════════════
  // SECTION 3: Query — legal doc summary (tier counts)
  // ══════════════════════════════════════
  ...
  // ══════════════════════════════════════
  // SECTION 3b: Query — legal doc details (full rows for Curator)
  // ══════════════════════════════════════
  ...
  // ══════════════════════════════════════
  // SECTION 3c: Query — escrow record (FC data)
  // ══════════════════════════════════════
  const { data: escrowRecord = null } = useQuery({
    ...
    queryFn: async (): Promise<EscrowRecord | null> => {
      const { data, error } = await supabase
        .from("escrow_records")
        .select(
          "id, escrow_status, deposit_amount, remaining_amount, bank_name, bank_branch, bank_address, currency, deposit_date, deposit_reference, fc_notes"
        )
        .eq("challenge_id", challengeId!)
        .maybeSingle();
