...
export default function AISpecReviewPage() {
  ...
  // ═══════ Effects — auto-repair disabled (solver types should start empty for user selection) ═══════
  // Previously auto-filled solver types from AI signals — now removed to let CR/CA select manually.
  ...
  if (!hasAiData && !isCR) {
    return (
      <div className="max-w-4xl mx-auto p-6 text-center py-16">
        ...
        <h2 className="text-lg font-semibold text-foreground">Waiting for {isMP ? 'Challenge Architect' : 'Challenge Creator'}</h2>
        <p className="text-sm text-muted-foreground mt-1 max-w-md mx-auto">
          The {isMP ? 'Challenge Architect' : 'Challenge Creator'} has not yet generated the AI specification for this challenge.
          Please check back later or contact the assigned {isMP ? 'architect' : 'creator'}.
        </p>
        <Button
          variant="outline"
          className="mt-5"
