...
export default function ShadowPricingPage() {
  ...
  const formFields: FormFieldConfig<FormData>[] = [
    ...
    { name: "currency_code", label: "Currency Code", type: "text", placeholder: "e.g., USD" },
  ...
  const columns: DataTableColumn<ShadowPricingWithJoins>[] = [
    ...
    { accessorKey: "currency_symbol", header: "Symbol" },
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    else await createM.mutateAsync(data as ShadowPricingInsert);
  };

  const defaults: Partial<FormData> = selected
    ? { tier_id: selected.tier_id, shadow_charge_per_challenge: selected.shadow_charge_per_challenge, currency_code: selected.currency_code, currency_symbol: selected.currency_symbol, description: selected.description, is_active: selected.is_active }
    : { tier_id: "", shadow_charge_per_challenge: 0, currency_code: "USD", currency_symbol: "$", is_active: true };
  ...
  const viewFields: ViewField[] = selected ? [
    ...
    { label: "Currency Code", value: selected.currency_code },
    { label: "Currency Symbol", value: selected.currency_symbol },
  ...
  return (
    <><div className="mb-6"><h1 className="text-2xl font-bold tracking-tight">Shadow Pricing</h1><p className="text-muted-foreground mt-1">Manage shadow billing charges per subscription tier for internal departments</p></div>
      ...
      <MasterDataForm open={isFormOpen} onOpenChange={setIsFormOpen} title="Shadow Pricing" fields={formFields} schema={schema} defaultValues={defaults} onSubmit={handleSubmit} isLoading={createM.isPending || updateM.isPending} mode={selected ? "edit" : "create"} />
