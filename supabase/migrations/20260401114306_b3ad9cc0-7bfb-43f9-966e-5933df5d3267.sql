...
-- Drop existing function with correct parameter order
DROP FUNCTION IF EXISTS public.complete_phase(UUID, UUID);

-- Recreate with Gate 03 added
...
BEGIN
  ...
  -- Step 2.5a: GATE VALIDATION for Phase 3 (Governance-aware)
  IF v_current_phase = 3 THEN
    v_gate_result := public.validate_gate_03(p_challenge_id);
    IF NOT COALESCE((v_gate_result->>'passed')::boolean, false) THEN
      RAISE EXCEPTION 'GATE-03 validation failed: %', v_gate_result->>'failures';
    END IF;
  END IF;

  -- Step 2.5b: GATE VALIDATION for Phase 4
  ...
  IF v_same_actor THEN
    -- Auto-advance: same actor can continue
    v_auto_completed := v_auto_completed || v_current_phase;
    ...
    INSERT INTO public.audit_trail (
      ...
      p_challenge_id, p_user_id, 'PHASE_AUTO_ADVANCED', 'SYSTEM',
      v_current_phase, v_next_phase,
      jsonb_build_object('reason', 'Same actor holds next role: ' || v_next_required_role)
    );

    -- Recurse to try completing the next phase too
    v_recursive_result := public.complete_phase(p_user_id, p_challenge_id);

    -- Merge auto_completed arrays
    ...
    RETURN jsonb_build_object(
      ...
      'phases_auto_completed', to_jsonb(COALESCE(v_auto_completed, '{}'::int[]))
    );
  ELSE
    -- Different actor needed: advance and hand off
    UPDATE public.challenges
    SET current_phase = v_next_phase,
        phase_status = 'ACTIVE',
        updated_at = NOW(),
    ...
    -- Find any user with the required role for notification
    SELECT ucr.user_id INTO v_next_user_id
    FROM public.user_challenge_roles ucr
    WHERE ucr.challenge_id = p_challenge_id
      AND ucr.role_code = v_next_required_role
    ...
    INSERT INTO public.audit_trail (
      ...
      v_current_phase, v_next_phase,
      jsonb_build_object(
        'reason', 'Handed off to role: ' || v_next_required_role,
        'next_user_id', v_next_user_id
      )
    );
...
GRANT EXECUTE ON FUNCTION public.complete_phase(UUID, UUID) TO authenticated;
