...
-- MOD-07 Gap Fix: Restore MOD-06 RPCs with canonical config keys + add atomic domain weights RPC + fix RLS

-- ============================================================================
-- GAP-1/GAP-2: Restore execute_auto_assignment with MOD-06 full logic + canonical keys
-- ============================================================================
...
DECLARE
  ...
  v_statuses TEXT[];
BEGIN
  -- Read weights from config (canonical keys from MOD-07)
  SELECT
...
-- ============================================================================
-- GAP-1: Restore reassign_verification with MOD-06 full logic + canonical keys
-- ============================================================================
...
-- ============================================================================
-- GAP-1: Restore bulk_reassign_admin with MOD-06 full logic
-- ============================================================================
...
-- ============================================================================
-- GAP-3: Atomic update_domain_weights RPC
-- ============================================================================
CREATE OR REPLACE FUNCTION public.update_domain_weights(
  p_l1 INTEGER,
  p_l2 INTEGER,
  p_l3 INTEGER,
...
BEGIN
  ...
  -- Validate sum = 100
  IF p_l1 + p_l2 + p_l3 <> 100 THEN
    RETURN jsonb_build_object('success', false, 'error', 'DOMAIN_WEIGHT_SUM_VIOLATION', 'detail', 'Weights must sum to 100');
  END IF;

  -- Validate range (0-100 each)
  IF p_l1 < 0 OR p_l1 > 100 OR p_l2 < 0 OR p_l2 > 100 OR p_l3 < 0 OR p_l3 > 100 THEN
    RETURN jsonb_build_object('success', false, 'error', 'OUT_OF_RANGE', 'detail', 'Each weight must be 0-100');
  END IF;

  -- Get old values
  SELECT param_value INTO v_old_l1 FROM md_mpa_config WHERE param_key = 'domain_weight_l1_industry';
  SELECT param_value INTO v_old_l2 FROM md_mpa_config WHERE param_key = 'domain_weight_l2_country';
  SELECT param_value INTO v_old_l3 FROM md_mpa_config WHERE param_key = 'domain_weight_l3_org_type';

  -- Update all three atomically
  UPDATE md_mpa_config SET param_value = p_l1::TEXT, updated_at = NOW(), updated_by_id = v_admin_id
   WHERE param_key = 'domain_weight_l1_industry';
  UPDATE md_mpa_config SET param_value = p_l2::TEXT, updated_at = NOW(), updated_by_id = v_admin_id
   WHERE param_key = 'domain_weight_l2_country';
  UPDATE md_mpa_config SET param_value = p_l3::TEXT, updated_at = NOW(), updated_by_id = v_admin_id
   WHERE param_key = 'domain_weight_l3_org_type';

  -- Insert audit rows
  INSERT INTO md_mpa_config_audit (param_key, previous_value, new_value, changed_by_id, change_reason, ip_address)
  VALUES
    ('domain_weight_l1_industry', v_old_l1, p_l1::TEXT, v_admin_id, p_change_reason, p_ip_address),
    ('domain_weight_l2_country', v_old_l2, p_l2::TEXT, v_admin_id, p_change_reason, p_ip_address),
    ('domain_weight_l3_org_type', v_old_l3, p_l3::TEXT, v_admin_id, p_change_reason, p_ip_address);
...
-- ============================================================================
-- GAP-5: Fix RLS to use admin_tier instead of is_supervisor
-- ============================================================================
DROP POLICY IF EXISTS "supervisor_modify_config" ON md_mpa_config;
CREATE POLICY "supervisor_modify_config" ON md_mpa_config
  FOR UPDATE TO authenticated
  USING (FALSE);

DROP POLICY IF EXISTS "supervisor_read_config" ON md_mpa_config;
CREATE POLICY "supervisor_read_config" ON md_mpa_config
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM platform_admin_profiles
      WHERE user_id = auth.uid()
        AND admin_tier IN ('supervisor', 'senior_admin', 'admin')
    )
  );
