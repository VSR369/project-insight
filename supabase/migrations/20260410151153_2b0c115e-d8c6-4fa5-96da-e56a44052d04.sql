...
-- ═══════════════════════════════════════════════════════════════════════════
-- Step 1: Drop the accidental duplicate overload causing PostgREST ambiguity
-- Signature: (uuid, text, uuid, text, text) — wrong arg order
-- ═══════════════════════════════════════════════════════════════════════════
DROP FUNCTION IF EXISTS public.initialize_challenge(uuid, text, uuid, text, text);

-- ═══════════════════════════════════════════════════════════════════════════
-- Step 2: Recreate canonical initialize_challenge with all fixes
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- AGG bypass: skip phase 1 (FIX: correct 3-arg call)
  IF v_is_agg THEN
    BEGIN
      PERFORM public.handle_phase1_bypass(v_challenge_id, v_operating_model, TRUE);
    EXCEPTION WHEN undefined_function THEN
      NULL;
    END;
  END IF;

  -- Start SLA timer (FIX: includes role_code and deadline_at)
  v_start_phase := CASE WHEN v_is_agg THEN 2 ELSE 1 END;

  SELECT required_role, COALESCE(sla_days, 5)
    INTO v_start_role, v_sla_days
    FROM public.md_lifecycle_phase_config
    WHERE governance_mode = v_governance
      AND phase_number = v_start_phase
      AND is_active = true
    LIMIT 1;

  BEGIN
    INSERT INTO public.sla_timers (
      challenge_id, phase, role_code, started_at, deadline_at, status, created_by
    ) VALUES (
      v_challenge_id,
      v_start_phase,
      COALESCE(v_start_role, 'CR'),
      NOW(),
      NOW() + make_interval(days => COALESCE(v_sla_days, 5)),
      'ACTIVE',
      p_creator_id
    );
    ...
    INSERT INTO public.audit_trail (
      ...
      jsonb_build_object('phase', v_start_phase, 'role', COALESCE(v_start_role, 'CR'), 'sla_days', v_sla_days)
    );
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Step 3: Harden complete_phase — wrap SLA insert in BEGIN/EXCEPTION
-- ═══════════════════════════════════════════════════════════════════════════
