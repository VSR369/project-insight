...
DROP FUNCTION IF EXISTS public.assemble_cpa(UUID, UUID);
...
BEGIN
  ...
  SELECT c.id, c.title, c.organization_id, c.tenant_id,
    ...
    c.solver_audience, c.submission_deadline,
  ...
  -- Prize: get from challenge_prize_tiers (Platinum / rank=1), fall back to total_fee
  SELECT COALESCE(
    pt.fixed_amount,
    CASE WHEN pt.percentage_of_pool > 0 AND COALESCE(v_ch.total_fee, 0) > 0
         THEN pt.percentage_of_pool * v_ch.total_fee / 100
         ELSE NULL END,
    v_ch.total_fee,
    0
  ) INTO v_prize
  ...
  IF v_prize IS NULL THEN
    v_prize := COALESCE(v_ch.total_fee, 0);
  END IF;

  -- Fetch org
  SELECT * INTO v_org FROM seeker_organizations WHERE id = v_ch.organization_id;

  -- Fetch CPA template for this governance mode
  ...
  -- Resolve geography via org country
  BEGIN
  ...
  -- Delete any existing assembled CPA for this challenge to allow re-runs
  DELETE FROM challenge_legal_docs
  WHERE challenge_id = p_challenge_id
    AND is_assembled = true
    AND document_type = v_cpa_code;

  -- Insert assembled CPA with valid tier and status
