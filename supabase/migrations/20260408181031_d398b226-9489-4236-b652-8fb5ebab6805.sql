-- ═══════════════════════════════════════════════════════════════
-- Fix C: get_required_legal_docs — support QUICK/STRUCTURED/CONTROLLED
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Resolve legacy mode names to canonical set
  v_resolved_mode := UPPER(TRIM(COALESCE(p_governance_profile, 'STRUCTURED')));
  IF v_resolved_mode = 'LIGHTWEIGHT' THEN v_resolved_mode := 'QUICK'; END IF;
  IF v_resolved_mode = 'ENTERPRISE' THEN v_resolved_mode := 'CONTROLLED'; END IF;
  IF v_resolved_mode NOT IN ('QUICK', 'STRUCTURED', 'CONTROLLED') THEN
    v_resolved_mode := 'STRUCTURED';
  END IF;

  -- TIER 1: match plain maturity level
  ...
  -- TIER 2: CONTROLLED gets enterprise-specific docs too
  IF v_resolved_mode = 'CONTROLLED' THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'template_id', template_id,
      'document_type', document_type,
      'document_name', document_name,
  ...
  ELSE
    -- QUICK / STRUCTURED: only plain maturity matches
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'template_id', template_id,
      'document_type', document_type,
      'document_name', document_name,
  ...
  RETURN jsonb_build_object(
    ...
    'governance_profile', v_resolved_mode
  );
END;
$function$;

-- ═══════════════════════════════════════════════════════════════
-- Fix D: complete_phase — add maturity filter to legal doc inserts
-- ═══════════════════════════════════════════════════════════════
-- We use a targeted replacement approach: drop and recreate just the
-- Phase 3 legal doc insert logic within complete_phase.
-- Since complete_phase is large, we recreate it fully with the fix applied.

-- First read the challenge maturity to filter templates.
-- The fix adds: AND (required_for_maturity ? v_maturity_key OR required_for_maturity IS NULL)
-- to the QUICK and STRUCTURED template insert blocks.
