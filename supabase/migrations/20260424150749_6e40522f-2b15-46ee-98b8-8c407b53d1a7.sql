...
BEGIN
  ...
  -- Advance to next phase. phase_status stays within the allowed set
...
-- Re-run the heal for the three challenges still stuck in Phase 1.
DO $$
DECLARE
  v_ids UUID[] := ARRAY[
...
BEGIN
  FOREACH v_id IN ARRAY v_ids LOOP
    BEGIN
      ...
      IF v_creator IS NULL OR v_phase IS NULL THEN CONTINUE; END IF;
