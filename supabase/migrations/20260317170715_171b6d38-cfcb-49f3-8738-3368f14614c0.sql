...
-- ============================================================
-- GATE-04: Phase 4 → Phase 5 validation gate
-- Called automatically by complete_phase when advancing from Phase 4.
-- Checks:
--   1. All 14 curation checklist items pass
--   2. No unresolved amendment_records
--   3. Complexity parameters entered and score calculated
--   4. Innovation Director explicit approval in audit_trail
-- Returns: {passed: boolean, failures: text[]}
-- ============================================================

CREATE OR REPLACE FUNCTION public.validate_gate_04(p_challenge_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  -- Fetch challenge
  SELECT id, complexity_score, complexity_parameters, complexity_level
  INTO v_challenge
  FROM challenges
  ...
  -- CHECK 1: All 14 curation checklist items pass
  v_checklist := validate_curation_checklist(p_challenge_id);
  IF NOT COALESCE((v_checklist->>'all_passed')::boolean, false) THEN
    -- Collect specific failed items
    SELECT array_agg(elem->>'label')
    INTO v_failed_items
    FROM jsonb_array_elements(v_checklist->'items') AS elem
    WHERE (elem->>'passed')::boolean = false;

    IF v_failed_items IS NOT NULL THEN
      FOREACH v_unresolved_count IN ARRAY ARRAY[]::INT[] LOOP NULL; END LOOP; -- no-op
      v_failures := v_failures || ARRAY['Curation checklist incomplete: ' || array_to_string(v_failed_items, ', ')];
    ELSE
      v_failures := v_failures || ARRAY['Curation checklist did not fully pass'];
    END IF;
  END IF;

  -- CHECK 2: No unresolved amendment_records
  ...
  IF v_unresolved_count > 0 THEN
    v_failures := v_failures || ARRAY[v_unresolved_count || ' unresolved amendment record(s) remain'];
  END IF;

  -- CHECK 3: Complexity parameters entered and score calculated
  IF v_challenge.complexity_score IS NULL THEN
    v_failures := v_failures || ARRAY['Complexity score has not been calculated'];
  END IF;

  IF v_challenge.complexity_parameters IS NULL THEN
    v_failures := v_failures || ARRAY['Complexity parameters have not been entered'];
  END IF;

  IF v_challenge.complexity_level IS NULL THEN
    v_failures := v_failures || ARRAY['Complexity level has not been determined'];
  END IF;

  -- CHECK 4: Innovation Director explicit approval in audit_trail
  SELECT EXISTS(
    SELECT 1
    FROM audit_trail
    WHERE challenge_id = p_challenge_id
      AND action IN ('COMPLEXITY_FINALIZED', 'CHALLENGE_APPROVED')
  ) INTO v_has_approval;

  IF NOT v_has_approval THEN
    v_failures := v_failures || ARRAY['Innovation Director has not explicitly approved this challenge'];
  END IF;
...
-- ============================================================
-- Update complete_phase to call validate_gate_04 before Phase 4 completion
-- ============================================================
...
BEGIN
  ...
  -- Step 2.5: GATE VALIDATION for Phase 4
  IF v_current_phase = 4 THEN
    v_gate_result := public.validate_gate_04(p_challenge_id);
    IF NOT COALESCE((v_gate_result->>'passed')::boolean, false) THEN
      RAISE EXCEPTION 'GATE-04 validation failed: %', v_gate_result->>'failures';
    END IF;
  END IF;
