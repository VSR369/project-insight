...
-- ═══════════════════════════════════════════════════
-- UPDATE complete_phase: AGG org templates + legal threshold logic
-- ═══════════════════════════════════════════════════
...
DECLARE
  ...
  v_operating_model TEXT; v_org_id UUID; v_prize_pool NUMERIC; v_threshold NUMERIC;
BEGIN
  ...
  IF v_next_phase = 3 THEN
    ...
    -- Load operating model and org context for template selection
    ...
    IF v_legal_doc_mode IN ('manual_review', 'ai_review') THEN
      BEGIN
        -- AGG model: use org-level templates; MP model: use platform templates
      ...
      -- Legal threshold check for STRUCTURED: below threshold → auto-approve
      IF v_gov_mode = 'STRUCTURED' THEN
        BEGIN
          SELECT COALESCE(ogo.legal_review_threshold_override, mlrt.threshold_amount, 50000)
          INTO v_threshold
          FROM public.challenges c
          LEFT JOIN public.org_governance_overrides ogo
            ON ogo.organization_id = c.organization_id
          ...
          LEFT JOIN public.md_legal_review_thresholds mlrt
            ON mlrt.country_id = so.hq_country_id::text
            AND mlrt.governance_mode = v_gov_mode AND mlrt.is_active = true
          WHERE c.id = p_challenge_id;

          IF v_prize_pool <= COALESCE(v_threshold, 50000) THEN
            UPDATE public.challenges SET lc_compliance_complete = TRUE WHERE id = p_challenge_id;
            -- Update inserted docs to curator_reviewed
            UPDATE public.challenge_legal_docs
            SET status = 'curator_reviewed', lc_status = 'approved'
            WHERE challenge_id = p_challenge_id AND status = 'pending_review';
          END IF;
        EXCEPTION WHEN OTHERS THEN NULL;
        END;
      END IF;
