/**
 * accept-role-invitation — Edge Function
 * Validates acceptance_token and activates the role assignment.
...
serve(async (req) => {
  ...
  try {
    ...
    if (updateError) {
      console.error("Failed to accept role invitation:", updateError.message);
      return new Response(
        JSON.stringify({ success: false, error: { code: "INTERNAL_ERROR", message: "Failed to accept invitation" } }),
    ...
    console.error("accept-role-invitation error:", error);
    return new Response(
