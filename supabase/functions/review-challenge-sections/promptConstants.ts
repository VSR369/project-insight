...
export const EXTENDED_BRIEF_FORMAT_INSTRUCTIONS: Record<string, string> = {
  ...
  expected_outcomes: 'Output: JSON array of objects: [{ "name": "Short outcome title", "description": "What changes — full SMART context", "acceptance_criteria": "Specific testable criteria proving this outcome was achieved" }]. Each MUST be SMART: what changes, by how much, measured how, by when. Bad name: "Improved efficiency". Good name: "Reduce processing time 12x". Description example: "Reduce processing time from 48hrs to <4hrs for 95% of transactions within 6 months of implementation". Acceptance criteria example: "System logs show median processing time <4hrs across 30-day rolling window; p95 <8hrs". Include both primary outcomes (directly from deliverables) and secondary outcomes (downstream business impact). All three fields are REQUIRED — never leave acceptance_criteria empty.',
  ...
  phase_schedule: 'Output: JSON array of phase objects with keys: phase_name (REQUIRED non-empty descriptive string — examples: "Registration", "Submission Window", "Evaluation", "Winner Announcement". NEVER leave blank, NEVER use "—", "TBD", or generic "Phase 1"/"Phase 2"), duration_days (number), start_date (ISO YYYY-MM-DD), end_date (ISO YYYY-MM-DD). Standard phases: Registration → Submission → Evaluation → Winner Announcement. Duration MUST align with challenge complexity and maturity: Blueprint = 4-8 weeks total, POC = 8-16 weeks, Pilot = 16-32 weeks. All dates MUST be in the future relative to today\'s date. Include buffer between phases for administrative processing.',
...
- Real-time pricing adjustments (separate initiative in Q3)`,
  ...
  problem_statement: `Our supply chain planning team currently generates demand forecasts using 18-month-old statistical models maintained in Excel spreadsheets. These models produce a Mean Absolute Percentage Error (MAPE) of 23%, significantly above the industry benchmark of 12-15% for comparable retail/CPG operations (Gartner Supply Chain Benchmark, 2024).

This forecasting gap drives two measurable impacts: (1) $4.2M in annual excess inventory carrying costs across our 12 North American distribution centers, and (2) 340 stockout events per quarter affecting 847 active SKUs, resulting in an estimated $1.8M in lost revenue per quarter.

We need a modern ML-based demand forecasting system that integrates with our SAP S/4HANA ERP, reduces MAPE to below 15%, and includes automated weekly model retraining to prevent future model degradation. The system must handle our full SKU catalog across all distribution centers with sub-60-second inference latency.

Key constraints: (a) Must integrate with existing SAP S/4HANA via standard APIs — no direct database access, (b) Must comply with our data residency requirements (US-only processing), (c) Must be maintainable by our 3-person data engineering team without specialized ML expertise.`,
  ...
  deliverables: `[{"name":"Demand Forecasting API","description":"REST API accepting SKU-level historical sales data (CSV/JSON), returning 13-week rolling forecasts with confidence intervals per SKU per distribution center. Documented with OpenAPI 3.0 specification.","acceptance_criteria":"API responds within 60 seconds for batch of 847 SKUs, returns valid JSON with forecast values and 80%/95% confidence intervals, handles missing data gracefully with documented imputation strategy.","format":"Code (Python/Docker container + OpenAPI spec)"},{"name":"Model Training Pipeline","description":"Reproducible pipeline for training, validation, and deployment of forecasting models. Must include feature engineering, hyperparameter tuning, and automated model selection.","acceptance_criteria":"Pipeline runs end-to-end on provided training data in under 4 hours, produces model with MAPE <15% on holdout set, includes automated cross-validation report.","format":"Code (Jupyter notebooks + pipeline scripts)"},{"name":"Integration Guide","description":"Technical documentation for connecting the forecasting API to SAP S/4HANA, including authentication, data mapping, error handling, and monitoring setup.","acceptance_criteria":"A mid-level engineer can follow the guide to set up the integration in under 8 hours. Includes troubleshooting section for common SAP connectivity issues.","format":"PDF (max 30 pages)"}]`,

  expected_outcomes: `["Reduce demand forecast MAPE from 23% to below 15% across all 847 SKUs, measured via automated holdout testing on 12-week rolling windows — achieving or exceeding the Gartner-reported industry median of 12-15% for retail/CPG","Cut quarterly stockout events from 340 to below 170 (50%+ reduction), measured via monthly SAP inventory exception reports across all 12 distribution centers","Reduce excess inventory carrying costs by minimum $2M annually (from current $4.2M baseline), measured via quarterly inventory aging reports","Establish automated weekly model retraining capability, eliminating the current 18-month model staleness cycle — measured by model age never exceeding 7 days in production"]`,
};
