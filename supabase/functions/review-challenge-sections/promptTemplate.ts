...
/** Industry code resolver — identity function, codes match master data directly. */
export function resolveIndustryCode(code: string | null | undefined): string | null {
  if (!code) return null;
  return code;
}

/** Map country code to geography region */
...
/** Build industry intelligence block for prompt injection */
export function buildIndustryIntelligence(
  ...
  // Regulatory landscape — select by region
  ...
  // Section-specific hints — ONLY for sections in this batch
...
/** Build geography context block for prompt injection */
