...
Rewrite the section content following the instructions. Return ONLY the refined content, nothing else.`;
    ...
    const EB_FORMAT_INSTRUCTIONS: Record<string, string> = {
      ...
      preferred_approach: `\n\nCRITICAL FORMAT REQUIREMENT: Return ONLY a valid JSON array of short phrase strings describing preferred approaches the seeker wants solvers to consider. Preserve the seeker's original intent. Refine existing items for clarity and consistency. Max 10 items. Example: ["AI/ML-based anomaly detection", "Cloud-native microservices architecture"]`,
